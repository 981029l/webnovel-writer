...
**语域要求（必须遵守）**：人物卡是会进入正文写作提示词的设定文本。性格、动机、能力的所有描述必须使用符合本题材世界观的语言；严禁用现代职场/财务/互联网词汇（职场、复盘、KPI、成本、性价比、数据化、项目、进度条等）描述人物特质。若主角是穿越者，"现代思维"只能作为思维方式的设定存在，文字表述必须转译为世界内语言（例：不写"现代职场式复盘思维"，写"凡事先掂量目标、代价与后手，三息之内权衡已定"）。"""
