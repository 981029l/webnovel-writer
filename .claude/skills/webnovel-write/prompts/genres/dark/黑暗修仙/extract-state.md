<!-- generated slot=extract_state gen_hash=1bda5cceeb -->
...
```json
{
  ...
  "status_file_updates": {
    ...
    "resource_updates": [
      {"resource_name": "资源名（如灵石、丹药、血食、炉鼎材料）", "new_value": "新值", "reason": "变化原因"}
    ],
    "troop_casualties": {
...
⚠️ **命名统一规则（必须遵守）**：
1. 若正文提到的功法/邪术与【当前已有功法/邪术】显然是同一招式（简称/别称/口语化写法），不要在 `new_techniques` 新建重复档案。
