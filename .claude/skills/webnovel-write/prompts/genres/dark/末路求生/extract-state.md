<!-- generated slot=extract_state gen_hash=2ec50e7664 -->
...
```json
{
  "new_characters": [
    {
      ...
      "identity": "身份（狱友/仇家/上位者/线人/同伙等）",
      "relation": "与主角关系",
      "appearance": "外貌描写",
      "personality": "性格特点",
      "realm": "当前实力层级或身份（用本书体系的说法，如血奴、执事、堂主）",
      "location": "当前地点（如下城区、刑房、废弃矿坑）",
      "first_action": "本章主要行为"
    }
  ],
  "new_treasures": [
  ...
  "status_changes": [
    {
      ...
      "realm": "最新实力层级（未变化填空字符串）",
      "location": "最新地点（未变化填空字符串）",
      "change": "状态变化简述（如残疾、背叛、上位、身份暴露）"
    }
  ],
  "entity_events": [
  ...
  "status_file_updates": {
    ...
    "resource_updates": [
      {"resource_name": "资源名（如口粮（按天数）、净水、药品、燃料）", "new_value": "新值", "reason": "变化原因"}
    ],
    "troop_casualties": {
      "dead_count": "死亡人数（数字或估算如'约500人'）",
      "wounded_count": "受伤人数",
      "surviving_count": "存活人数（如果正文提到）",
      "unit_name": "团伙/队伍名称（用正文中的称呼）",
      "description": "伤亡描述"
    },
    "new_items": [
...
⚠️ **命名统一规则（必须遵守）**：
1. 若正文提到的手段/功法与【当前已有手段/功法】显然是同一招式（简称/别称/口语化写法），不要在 `new_techniques` 新建重复档案。
