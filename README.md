...
- **🗂️ 大纲编辑** - 树状图可视化编辑卷纲和章纲，内建章节连贯性硬约束与跳号断层自动检测机制（防范跳章断号）
...
- 番茄小说自动发布：多账号 cookie 管理、书名精确匹配、章节自动发布
- 写作流程增强：增加审查修复循环，写作完成后自动审查并修复问题
- 设定同步：大纲生成后自动同步主角档案到角色库
- 大纲提取角色增加 importance 字段，修复反派角色分类缺失
- 删除分卷时联动删除宝物/功法/势力/地点档案
- PM2 统一运行脚本，简化前后端进程管理
- AI 状态栏增强，新增剪贴板工具
- 修复中文引号导致的 Vue 解析错误
